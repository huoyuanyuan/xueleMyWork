...
$(function () {
    ...
    var pageHandle = {
        //添加body最小高度
        addBodyMinHeight: function () {
            var screenHeight = window.innerHeight
            $("body").css("minHeight",screenHeight)
        },
        //添加左边导航栏样式
        addLeftNavCss: function () {
    ...
    var addEventListeners = {
        //全选按钮监听
        addCheckAllBtnListener: function () {
    ...
    var ASadminHandle = {
        ...
        getArea: function () {
            ...
            self.getAreaByCode("", function (data) {
                self.addOption(data,"province_selector");
            });
            $("#province_selector").on("change", function () {
                var clickTargetVal = $(this).val();
                if(clickTargetVal == "province"){
                    regionSelector.hide();
                    self.removeOption("region_selector")
                    districtSelector.hide();
                    self.removeOption("district_selector");
                    ...
                    self.removeOption("region_selector");
                    self.removeOption("district_selector");
                    self.getAreaByCode(clickTargetVal, function (data) {
                        self.addOption(data,"region_selector");
                    })
                }
            });
            $("#region_selector").on("change", function () {
                ...
                if(clickTargetVal == "region"){
                    ...
                    self.removeOption("district_selector");
                    self.getAreaByCode(clickTargetVal, function (data) {
                        self.addOption(data,"district_selector");
                    })
                }
            });
        },
        ...
        getDetailAdress: function () {
            ...
            $("#getDetailBtn").on("click", function (e) {
                ...
                if(code.length == 2){
                    ...
                    self.getAreaByCode(selectCode, function (data) {
                        data.wrapper.forEach(function (item) {
        ...
        makeAS: function () {
            ...
            $("#AS_makeAS_saveBtn").on("click", function () {
                ...
                for(var j= 0,roleLength=ASTargetRole.length;j<roleLength;j++){
                    ...
                    arrObj.roleName = self.roleToRole(ASTargetRole.eq(j).attr("id"));
                    roleArr.push(arrObj);
                }
                var ASData = {
                    AS_name:ASName,
